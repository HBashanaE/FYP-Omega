...
neural_model_path = join(root_dirname, '../Data/neural-model-base-char.pth')
...
def get_accuracy(error, original):
    error = ['<s>'] + list(error) + ['</s>']
    original = ['<s>'] + list(original) + ['</s>']
...
with open(join(root_dirname, "result_evaluation_language_model_ocr.json"), "w", encoding='utf-8') as outfile:
    json.dump({"results": results, "final_socre": difference/i}, outfile, ensure_ascii=False)
