...
neural_model_path = join(
    root_dirname, f'../Data/neural-model-base-char.pth')
...
def get_accuracy(error, original):
    error = ['<s>'] + list(preprocess(error)) + ['</s>']
    original = ['<s>'] + list(preprocess(original)) + ['</s>']
...
with open(join(root_dirname, f"temp_result_base/result_evaluation_language_model_ocr_{batch_size}_{n_hidden}_{n_layers}.json"), "w", encoding='utf-8') as outfile:
...
with open(join(root_dirname, f"temp_result_base/result_evaluation_language_model_edit_distance_{batch_size}_{n_hidden}_{n_layers}.json"), "w", encoding='utf-8') as outfile:
    json.dump({"results": results,
               "final_socre": {
                   "difference_avg": difference_avg/(i + 1),
                   "difference_mul": difference_mul/(i + 1),
...
with open(join(root_dirname, f"temp_result_base/result_evaluation_language_model_random_{batch_size}_{n_hidden}_{n_layers}.json"), "w", encoding='utf-8') as outfile:
    json.dump({"results": results,
               "final_socre": {
                   "difference_avg": difference_avg/(i + 1),
                   "difference_mul": difference_mul/(i + 1),
